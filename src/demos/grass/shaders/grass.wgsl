...
fn vertMain(input: VertexInput) -> VertexOutput {
    ...
    /*
    var pos = input.position;
    var id = f32(input.InstanceIndex) / f32(totalInstances);

    var totalInstancesSqrt = sqrt(f32(totalInstances));
    var id2 = vec2f(id % f32(totalInstancesSqrt), id / f32(totalInstancesSqrt));
    ...
    var power = 2.;
    var offset = pow(2.718, (power * (1. - input.uv.x)) - power);
    pos.x += offset * 1.5 * (rand(id) * 2. - 1.); //  * sin(time + (rand(input.InstanceIndex) * 0.5 * (rand(input.InstanceIndex) + 1. * 10.)));

    // square position
    var groupiness = .2;

    pos.x += f32(totalInstancesSqrt/(2. / groupiness)) - (groupiness *  (id % f32(totalInstancesSqrt))  );
    pos.z += f32(totalInstancesSqrt/(2. / groupiness)) - (groupiness *  (id / f32(totalInstancesSqrt))  );

    // position offset
    pos.x += 2. * (rand(id) * 2. - 1.);
    pos.z += 2. * (rand(id + f32(totalInstances)) * 2. - 1.);
